...
(function() {
	...
	(window.EJSC = {
      ...
      // Disable SVG axis label on IE 6-8 and Firefox 2
      EJSC.__useSVGAxisLabel = !(EJSC.__isIE6To8 || /Firefox\/2\./.test(navigator.userAgent));
			...
			try {
				for (var i = 0; i < metaTags.length; i++) {
					if (metaTags[i].name == "ejsc-blank-image-path") {
            EJSC.blankImagePath = metaTags[i].content;
          } else if (metaTags[i].name == "ejsc-excanvas-path") {
            EJSC.excanvasPath = metaTags[i].content;
          } else if (metaTags[i].name == "ejsc-stylesheet-path") {
            EJSC.stylesheetPath = metaTags[i].content;
          } else if (metaTags[i].name == "ejsc-compatibility-file-path") {
            EJSC.compatibilityFilePath = metaTags[i].content;
			...
			EJSC.__linkTag.href = EJSC.stylesheetPath;
		...
		__deleteChart: function(index, clearEl, detachEvents) {
			...
			try {
				...
				if (detachEvents == undefined || detachEvents == true) {
					var e, i;
					for (var i = 0; i < EJSC.__events.length; i++) {
						e = EJSC.__events[i];
						if (e != null) {
							if (e[4] == chart) {
								EJSC.utility.__detachEvent( e[0] , e[1] , e[2] , e[3] );
		...
		__doResize: function() {
			...
			var i;
			for (var i = 0; i < EJSC.__ResizeTimeouts.length; i++) {
		...
		utility: {
			...
			XMLRequestPool: {
				...
				__returnHttpRequest: function(request) {
					...
					if (this.__requestPool.length < this.MaxPoolSize) { this.__requestPool.push(request); }
				},
		...
		Chart: function( id , options ) {
			...
			this.blank_image_path   = EJSC.blankImagePath;
		...
		__Formatter: function() {
			...
			this.format = function( value , roundVal ) {
			  return value.toPrecision(Math.max(3, roundVal || 0));
			};
		},
		//	--------------------------------------------------------------------------
		//	CLASS: __Series
		...
		__Series: {
						...
						["div", { className: "ejsc-legend-series" + (this.legendIsVisible?"":" ejsc-hidden") },
							["div", {
								...
								["div", { className: "ejsc-legend-series-icon", __ref: self, __refVar: "__legend_icon_container" },
									["img", {
										src: this.__getChart().blank_image_path,
			...
			__findClosestPoint: function(mouse, use_proximity) {
				if (!this.visible || !this.__getHasData() || !this.selectable) { return null; }
			...
			// DB: Added to prevent selection of points from series
			selectable: true,
			x_axis: "bottom",
			y_axis: "left",
		...
		__Axis: {
			...
			__doWrite:					undefined,
			__attemptToCreateVerticalSVGCaption: function() {
				if (EJSC.__useSVGAxisLabel && this.__orientation == 'v') {
					try {
						this.__el_measure = document.createElement("span");
						this.__el_measure.setAttribute("class", "ejsc-v-axis-caption-measurement");
						this.__el_measure.appendChild(document.createTextNode(this.caption));
						document.body.appendChild(this.__el_measure);
  					var captionWidth = this.__el_measure.offsetWidth;
  					var captionHeight = this.__el_measure.offsetHeight;

						var svgroot = document.createElementNS("http://www.w3.org/2000/svg", "svg");
						this.__el_svg_caption = document.createElementNS("http://www.w3.org/2000/svg", "text");
						svgroot.setAttribute("height", captionWidth);
						this.__el_svg_caption.setAttribute("y", captionHeight);
						this.__el_svg_caption.setAttribute("transform", "rotate(-90)");
						this.__el_svg_caption.setAttribute("text-anchor", "end");
						this.__el_svg_caption.appendChild(document.createTextNode(this.caption));
						svgroot.appendChild(this.__el_svg_caption);
						this.__el_caption.innerHTML = '';
						this.__el_caption.appendChild(svgroot);
					} catch (e) {}
				}
			},
			__updateCaption: function() {
				if (this.__el_svg_caption) {
					var unescapedCaption = this.__unescapeHTML(this.caption); // required because we are using createTextNode instead of setting innerHTML

					this.__el_measure.replaceChild(document.createTextNode(unescapedCaption), this.__el_measure.childNodes[0]);
					var captionWidth = this.__el_measure.offsetWidth;
					var captionHeight = this.__el_measure.offsetHeight;

					this.__el_svg_caption.parentNode.setAttribute("height", captionWidth);
					this.__el_svg_caption.setAttribute("y", captionHeight);
					this.__el_svg_caption.replaceChild(document.createTextNode(unescapedCaption), this.__el_svg_caption.childNodes[0]);
				} else {
				  this.__el_caption.innerHTML = this.__getCaption();
				}
			},
			// CDD: because SVG just uses text nodes, we need a way to unescape captions. Ugly!
			__stripTags: function(input) {
				if ('string' != typeof(input)) {
					input = '';
				}
			  return input.replace(/<\w+(\s+("[^"]*"|'[^']*'|[^>])+)?>|<\/\w+>/gi, '');
			},
			__unescapeHTML: function(input) {
			  return this.__stripTags(input).replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&').replace(/&quot;/g, '"').replace(/&#x27;/g, "'");
			},
			__write: function() {
				...
				this.__el = EJSC.utility.__createDOMArray(
					["span", {
						...
						["span", {
							...
							__styles: {
								filter: (EJSC.__isIE6To8) ? (this.__orientation=="v"?(this.__side=="left"?"flipV flipH":"flipV flipV"):"") : ""
							}
						}],
						["span", {
				...
				this.__attemptToCreateVerticalSVGCaption();
			...
			__removeSeries: function(series) {
				...
				if (this.__text_values && series.__points != undefined) {
					for (var i = 0; i < series.__points.length; i++) {
...
*/					["div", {
				...
				["div", { //CDD: Made these 2 elements div in place of span.
					className: "ejsc-hint",
					__ref: self,
					__refVar: "__el_hint"
				},
		...
		// Transform canvas tags if IE 6-8
		if (EJSC.__isIE6To8){
				...
				if (this.allow_zoom) {
					...
					if (this.onUserBeginZoom != undefined) {
						if (this.onUserBeginZoom(this) == false) {
							this.__zooming = false;
							return cancelEvent(e); // CDD: Cancel event to fix a bug where Firefox thinks it is selecting text
						}
					}
				}
	...
	___chart.__doMouseUpCanvasCover = function( e ) {
		...
		try {
		  ...
		  return false; // CDD: used to return false in the finally overriding any other return
		} finally {
			// Hide zoom box
			this.__doHideZoomBox();
	...
	___chart.__selectPoint = function(point, forceSticky, fireAfterEvent) {
		...
		// CDD: Don't trigger events over and over if the mouse is still on the same point
		if (point == undefined || point.__owner == undefined || point === this.selected_point) return;
		...
		if (this.selected_point != undefined && this.onAfterSelectPoint && fireAfterEvent != false) {
			...
			this.onAfterSelectPoint(point, point.__owner, this, this.__el_hint, (this.__hint_is_sticky==true||forceSticky?"select":"hover"));
		}
	...
	___chart.redraw = function(reselectPoint) {
		...
		var chart = this;
		window.setTimeout(function() { chart.draw(reselectPoint); },0);
	};

  // CDD: Extracted ability to draw synchronously
	___chart.draw = function(reselectPoint) {
    this.__resize(false, true, true);
    this.__draw(reselectPoint);
	};

  // CDD: Redraw axes only
  ___chart.redrawAxes = function() {
    this.__draw_axes(this.__axes_context, true);
  };

  // CDD: Extracted function to reset the zoom programatically
  ___chart.resetZoom = function(redraw) {
		try {
			if (this.onBeforeZoom != undefined) {
				if (this.onBeforeZoom(this) == false) { return false; }
			}
  ...
  // CDD: Clear all series
	___chart.clearSeries = function(redraw) {
    for (var i = this.__series.length - 1; i >= 0; i--) this.__removeSeries(this.__series[i], false);
	...
	___linearaxis.__initTicks = function() {
		...
		if (ticks.length > this.__el_labels.childNodes.length) {
			for (var i = this.__el_labels.childNodes.length; i <= ticks.length; i++) {
			...
			for (var i = this.__el_labels.childNodes.length-1; i >= ticks.length; i--) {
	...
	___logarithmicaxis.__initTicks = function() {
		...
		if (tick_count > this.__el_labels.childNodes.length) {
			for (var i = this.__el_labels.childNodes.length; i <= tick_count; i++) {
				this.__el_labels.appendChild(EJSC.utility.__createDOMArray(["span", { className: "ejsc-" + this.__orientation + "-label ejsc-invisible" + (this.label_class!=undefined?" " + this.label_class:"") }]));
			}
		} else if (tick_count < this.__el_labels.childNodes.length) {
			for (var i = this.__el_labels.childNodes.length-1; i >= tick_count; i--) {
				this.__el_labels.childNodes[i].className = "ejsc-" + this.__orientation + "-label ejsc-invisible" + (this.label_class!=undefined?" " + this.label_class:"");
			}
		}
	...
	EJSC.ScatterPoint = function(x, y, err, label, data, owner) {
		this.__owner 		= owner;
		this.x 				= (x.__label == undefined?x:x.__label);
		// JHM: 2008-01-11 - Added parseFloat to ensure y is recognized as a number
		//this.y				= (y.__label == undefined?parseFloat(y):y.__label);
		...
		// CDD Added for standard deviation
		this.err = (err == null?null:(err.__label == undefined?err:err.__label));
		this.__err_label = err;
		this.__err = function() {
			if (this.err == null ) { this.__err = function() { return null; }; }
			else if (this.__err_label.__label == undefined) { this.__err = function() { return this.err; }; }
			else { this.__err = function() { return this.__err_label.__index; }; }
			return this.__err();
		}
	};
	EJSC.XYPoint.prototype.__extendTo(EJSC.ScatterPoint);
	...
	___lineseries.__doAbstractLineDraw = function( ctx , seriesType , drawMode , openClose ) {
		...
		if (j < plen) {
			...
			while( j <  plen ) {
				...
				if (++pointsDrawn > 5000 && EJSC.__isIE6To8) {
					if( drawMode == 'line') {
						ctx.stroke();
						ctx.beginPath();
	...
	// CDD: Acquires a point from another line series
	___lineseries.acquirePoint = function(point) {
	  var originSeries = point.__owner;
    var idx = originSeries.__points.indexOf(point);
    if (idx!=-1) originSeries.__points.splice(idx, 1);
    originSeries.__hasData = (originSeries.__points.length > 0); // updates series
	...
	//	--------------------------------------------------------------------------------------------
	//	Supported pointStyle - circle, box, cross, diamond, triangle
	...
	___scatterseries.__doSetDataHandler = function(handler, reload) {
	...
	___scatterseries.__doOnDataAvailable = function(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (data[i].err === "") {
				data[i].err = null;
			} else if (data[i].err != null) {
				data[i].err = parseFloat(data[i].err);
			}

			this.__points.push(new EJSC.ScatterPoint(data[i].x, data[i].y, data[i].err, data[i].label, data[i].userdata, this));
		}
	...
	___scatterseries.__doDraw = function(ctx) {
		...
		try {
			...
			// CDD: Use Canvas pixel API if available
      var renderer = (this.__owner.usePixelApi && ctx.putImageData !== undefined) ? new PixelApiRenderer(ctx, ps, this.pointStyle, this.color, this.opacity) : new LineRenderer(ctx, ps, this.pointStyle, this.color, this.opacity);
            ...
            renderer.drawPoint(plotX, plotY);
						if (this.__points[j].__err()) {
							var high = y_axis.__pt2px(this.__points[j].__y() + this.__points[j].__err());
							var low = y_axis.__pt2px(this.__points[j].__y() - this.__points[j].__err());
							renderer.drawErrorBar(plotX, high, low);
						}
					}
			...
			if (this.orientation == "vertical") {
				...
				if (firstPoint < pointsLength) {
					...
					for (var r = 0; r <= this.ranges.length; r++) {
						...
						var plotYmax, plotYmin, plotXmax, plotXmin;

						if (EJSC.__isIE6To8 && (this.onBarNeedsColor == undefined) && !this.useColorArray) ctx.beginPath();
						...
						while (j < pointsLength && points[j].__x() <= seriesMaxX) {
							...
							if( !EJSC.__isIE6To8  || (this.onBarNeedsColor != undefined) || this.useColorArray) {
								ctx.closePath();
						...
						if (EJSC.__isIE6To8 && pointsDrawn && (this.onBarNeedsColor == undefined) && !this.useColorArray) {
							ctx.closePath();
	...
	EJSC.FunctionSeries = function( fn , options ) {
		...
		this.__function		= fn;
		// CDD: Default values would cause the plot to resize when adding/removing function series
		this.__minX 		= undefined;				// Initialize X Min extreme
		this.__maxX 		= undefined;				// Initialize X Max extreme
		this.__minY 		= undefined;				// Initialize Y Min extreme
		this.__maxY 		= undefined;				// Initialize Y Max extreme
	...
	___functionseries.__doDrawPlot = function(ctx) {
		...
		try {
			...
			for (var i = draw_area.left; i < draw_area.right; i++) {
				x = x_axis.__px2pt(i);
				y = fn(x);
	...
	___trendseries.computeLinearTrend = function() {
		...
		//	Store Function
		this.__function = function(x) { return b * x + a }
	};
	...
	___trendseries.computePowerTrend = function() {
		...
		//	Store Function
		this.__function = function(x) { return a * Math.pow(x, b) }
	};
	...
	___trendseries.computeExponentialTrend = function() {
		...
		//	Store Function
		this.__function = function(x) { return a * Math.pow(Math.E, b * x) }
	};
	...
	___trendseries.computeLogarithmicTrend = function() {
		...
		//	Store Function
		this.__function = function(x) { return a + b * Math.log(x); }
	};
  ...
  // CDD: factored out drawing of scatter plot
  class LineRenderer {
    constructor(context, pointSize, pointStyle, color, opacity) {
      this.context = context;
      this.pointStyle = pointStyle;

      switch (pointStyle) {
  			case "box": context.lineCap = "square"; pointSize = pointSize * 2; break;
  			case "circle": context.lineCap = "round"; pointSize = pointSize * 4; break;
        case "cross": context.lineCap = "square"; break;
        case "diamond": context.lineCap = "square"; break;
  			case "triangle": context.lineCap = "butt"; break;
  		}

  		context.lineWidth = this.pointSize = pointSize / 2;
  		this.r = pointSize / 4;

  		context.strokeStyle = EJSC.utility.__getColor(color, (opacity / 100)).rgba;
    ...
    drawPoint(x, y) {
      // JHM: Added to support larger data sets in IE
  		if (EJSC.__isIE6To8 && ++this.pointsDrawn > 4000) {
  			this.context.stroke();
  			this.pointsDrawn = 0;
  		...
  		switch(this.pointStyle) {
  			case 'box':
					this.context.moveTo(x + this.r , y + this.r);
					this.context.lineTo(x - this.r , y + this.r);
					this.context.lineTo(x - this.r , y - this.r);
					this.context.lineTo(x + this.r , y - this.r);
					this.context.lineTo(x + this.r , y + this.r);
					this.context.lineTo(x - this.r , y + this.r); // Draw an additional useless line to fix a bug where the bottom right of the box was not filled.
  				break;
  			case 'circle':
          this.context.moveTo(x-0.1,y);
          this.context.lineTo(x,y);
  				break;
  			case 'cross':
  			  this.context.moveTo(x - this.r * 4, y - this.r * 4);
  			  this.context.lineTo(x + this.r * 4, y + this.r * 4);
  			  this.context.moveTo(x - this.r * 4, y + this.r * 4);
  			  this.context.lineTo(x + this.r * 4, y - this.r * 4);
          break;
  			case 'diamond':
  				this.context.moveTo(x, y - this.r);
  				this.context.lineTo(x - this.r, y);
  				this.context.lineTo(x, y + this.r);
  				this.context.lineTo(x + this.r, y);
  				this.context.lineTo(x, y - this.r);
  				break;
  			case 'triangle':
		...
		drawErrorBar(x, high, low) {
			this.context.stroke();
			this.context.lineWidth = 1;
			this.context.beginPath();

			this.context.moveTo(x - this.r * 2, low);
			this.context.lineTo(x + this.r * 2, low);
			this.context.moveTo(x, low);
			this.context.lineTo(x, high);
			this.context.moveTo(x - this.r * 2, high);
			this.context.lineTo(x + this.r * 2, high);
   ...
   class PixelApiRenderer {
    constructor(context, pointSize, pointStyle, color, opacity) {
      this.context = context;
      this.colorData = EJSC.utility.__getColor(color, opacity / 100);

      var size = this.computeImageDataSize(pointSize, pointStyle);
      this.pointImageData = context.createImageData(size, size);
      this.setShape(pointStyle);
    }

    drawPoint(x, y) {
      this.context.putImageData(this.pointImageData, x, y);
    }
    ...
    computeImageDataSize(pointSize, pointStyle) {
      var result;
    ...
    setCircleShape() {
      var i;
      var grayscaleIntensities;

      switch(this.pointImageData.data.length) {
        case 9*4:
          grayscaleIntensities = [
            129,  47, 118,
             20,   0,   6,
            182, 109, 174
          ];
        ...
        case 36*4:
          grayscaleIntensities = [
            250, 224, 177, 219, 250, 250,
            174,   6,   0,   3, 159, 250,
             27,   0,   0,   0,  11, 246,
              5,   0,   0,   0,   0, 238,
            104,   0,   0,   0,  85, 250,
            247, 140,  83, 131, 243, 250
          ];
          break;
        case 121*4:
          grayscaleIntensities = [
            250, 250, 250, 203, 122,  95, 114, 187, 250, 250, 250,
            250, 247, 103,   0,   0,   0,   0,   0,  74, 237, 250,
            250, 115,   0,   0,   0,   0,   0,   0,   0,  75, 250,
            224,   2,   0,   0,   0,   0,   0,   0,   0,   0, 188,
            157,   0,   0,   0,   0,   0,   0,   0,   0,   0, 116,
            139,   0,   0,   0,   0,   0,   0,   0,   0,   0,  99,
            172,   0,   0,   0,   0,   0,   0,   0,   0,   0, 131,
            239,  19,   0,   0,   0,   0,   0,   0,   0,   1, 215,
            250, 167,   0,   0,   0,   0,   0,   0,   0, 128, 250,
            250, 250, 173,  27,   0,   0,   0,  13, 145, 250, 250,
            250, 250, 250, 247, 194, 165, 186, 242, 250, 250, 250
          ];
          break;
      }

      for(i = 0; i < grayscaleIntensities.length; i++) {
        // NOTE: can't just set pointImageData.data to some array, because in Safari and Chrome data is actually a CanvasPixelArray, using a normal array doesn't work
        this.setPointImageDataRGBA(i, grayscaleIntensities[i]);
      }
    }

    setPointImageDataRGBA(index, grayscaleIntensity) {
      var opacity = 255 - grayscaleIntensity;
      this.pointImageData.data[4 * index + 0] = this.colorData.red - grayscaleIntensity;
      this.pointImageData.data[4 * index + 1] = this.colorData.green - grayscaleIntensity;
      this.pointImageData.data[4 * index + 2] = this.colorData.blue - grayscaleIntensity;
      this.pointImageData.data[4 * index + 3] = this.colorData.opacity * opacity;
    }
  };
})();
