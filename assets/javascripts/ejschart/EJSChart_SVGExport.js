...
(function() {
		...
		try {
			...
			if (j < plen) {
				...
				switch (this.pointStyle) {
					...
					case "cross": ctx.lineCap = "square"; break;
				...
				while (j < plen) {
					...
					const plotX = x_axis.__pt2px(this.__points[j].__x());
					const plotY = y_axis.__pt2px(this.__points[j].__y());
					...
					if ((plotY + ps) >= canvas_top && (plotY - ps) <= canvas_bottom) {
						...
						switch(this.pointStyle) {
							case 'box':
								...
								ctx.lineTo( plotX + r , plotY - r );
								ctx.lineTo( plotX + r , plotY + r );
								ctx.lineTo( plotX - r , plotY + r ); // Draw an additional useless line to fix a bug where the bottom right of the box was not filled.
								break;
							case 'circle':
								// CDD: 2009-12-10 - use CDD's circle function instead of moveTo and arcs
								ctx.circle(plotX, plotY, r);
								break;
							case 'cross':
                ctx.moveTo(plotX - r * 4, plotY - r * 4);
                ctx.lineTo(plotX + r * 4, plotY + r * 4);
                ctx.moveTo(plotX - r * 4, plotY + r * 4);
                ctx.lineTo(plotX + r * 4, plotY - r * 4);
                break;
							case 'diamond':
						...
						if (this.__points[j].__err()) {
							...
							ctx.moveTo(plotX - r * 2, low);
							ctx.lineTo(plotX + r * 2, low);
							ctx.moveTo(plotX, low);
							ctx.lineTo(plotX, high);
							ctx.moveTo(plotX - r * 2, high);
							ctx.lineTo(plotX + r * 2, high);
	...
	var __textSize = function(aText, aFontFamily, aFontSize, aFontWeight, aFontStyle) {
    	...
    	// CDD: 2009-12-10 - don't round (by removing use of mr function) - makes the SVG larger but also makes the SVG not have weird rounding related problems
			ret.width += ret.width * 0.07;
		}
	...
	var ___svgcanvas = EJSC.SVGCanvas.prototype;
  ...
  // CDD: added circle function for drawing circular points more exactly than arc did
  ___svgcanvas.circle = function(aX, aY, aRadius) {
    this.currentPath_.push({type: "arcTo",
      radius: aRadius,
      start: {x: aX - aRadius, y: aY},
      end: {x: aX + aRadius , y: aY},
      largeArc: 1,
      clockwise: 1
    });
	...
	___svgcanvas.stroke = function(aFill) {
		...
		for (var i = 0, p = this.currentPath_[i], n = this.currentPath_[i+1]; i < this.currentPath_.length; i++, p = this.currentPath_[i]) {
			if((p.type == "moveTo") && ((n && n.type != "arcTo") || !n)) {
			  // CDD: 2009-12-10 - don't round (by removing use of mr function) - makes the SVG larger but also makes the SVG not have weird rounding related problems
				lineStr.push(" M", p.x, ",", p.y);
				canClose = true;
			} else if (p.type == "lineTo") {
			  // CDD: 2009-12-10 - don't round (by removing use of mr function) - makes the SVG larger but also makes the SVG not have weird rounding related problems
				lineStr.push(" L", p.x, ",", p.y);
				canClose = true;
			} else if ((p.type == "close") && canClose){
				lineStr.push(" Z");
			} else if (p.type == "arcTo") {
			  // CDD: 2009-12-10 - don't round (by removing use of mr function) - makes the SVG larger but also makes the SVG not have weird rounding related problems
				lineStr.push(" M", p.start.x, ",", p.start.y, " A", p.radius, ",", p.radius, " 0 ", p.largeArc, ",", p.clockwise, " ", p.end.x, ",", p.end.y);
				canClose = false;
			}
		}
    ...
    var viewBoxX1 = 0 - marginLeft;
    var viewBoxY1 = 0 - marginTop;
    var viewBoxX2 = this.width + marginRight;
    var viewBoxY2 = this.height + marginBottom;

		var height		= (aHeight != undefined) ? aHeight : this.height + marginTop + marginBottom;
		var width		= (aWidth != undefined) ? aWidth : this.width + marginLeft + marginRight;
    ...
    var openTag   = '<' + tagSVG + ' width="' + width + '" height="' + height + '" viewBox="' + viewBoxX1 + ' ' + viewBoxY1 + ' ' + viewBoxX2 + ' ' + viewBoxY2 + '" version="1.1" xmlns' + ((this.nameSpace) ? ':' + this.nameSpace : '') + '="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\r\n';
    var background = '<rect x="' + viewBoxX1 + '" y="' + viewBoxY1 + '" width="' + viewBoxX2 + '" height="' + viewBoxY2 + '" fill="white" />';
		var closeTag	= '</' + tagSVG + '>';
		...
		return header + openTag + pathDefine + background + pathDraw + textDraw + closeTag;
	...
	___chart.__exportSVGTitleBar = function(ctx) {
		...
		ctx.drawText(titlebar.offsetLeft, titlebar.offsetTop + text.offsetHeight, titlebar.offsetWidth, titlebar.offsetTop + text.offsetHeight, text.innerHTML, __getCurrentStyle(titlebar, "text-align"));
